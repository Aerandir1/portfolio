    ...
    projects: [
      ...
      { 
        ...
        action: {
          ...
          title: "Gravity Sim",
          url: "/projets/SolarSystem/build/web/index.html"
        },
        imageMaxWidth: 520,
        imageBorderRadius: 12
      },
...
function renderProjects() {
  ...
  projects.forEach((p) => {
    ...
    const imageHtml = p.image ? `<div class='project-image-block'><img src="${p.image}" alt="${p.title}" ${imgStyle}></div>` : '';
    const actionHtml = p.action
      ? `<button class="project-action-btn" type="button" data-project-action="${p.id}">${p.action.label}</button>`
      : '';

    let detailsHtml = `
      <div class='project-structure'>
        <div class='project-desc-block'><p class='project-desc'>${p.details}</p></div>
        ${imageHtml}
        ${techs}
        <div class='project-actions'>${actionHtml}</div>
      </div>
    `;
    ...
    const actionButton = card.querySelector('.project-action-btn');
    if (actionButton) {
...
//   PROJECT MODAL
// =========================
function openProjectModal({ title, url, note = '' }) {
  ...
  modal.innerHTML = `
    <div class='sim-modal-content'>
      ...
      <h2>${title}</h2>
      ${note ? `<p class="sim-modal-note">${note}</p>` : ''}
      <iframe src="${url}" scrolling="no" style="border:0;width:100%;max-width:1000px;aspect-ratio: 16 / 9; height: auto; display:block; background: transparent;"></iframe>
    </div>
  `;
  modal.querySelector('.sim-modal-close').addEventListener('click', closeProjectModal);
